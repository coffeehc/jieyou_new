...
class Index {
    ...
    public function index() {
        $config = [
            'app_key' => 'wx3b3cd9b60e250226',
            'app_secret' => '377cb67b5891326c13f83b94602f0f93',
            'scope' => 'snsapi_login',
            'callback' => [
                'default' => 'http://www.175jy.com/wxlogin/index/callback',
            ]
        ];
    ...
    public function callback() {
        ...
        if(!$user) {
            ...
            $insertData['users'] = preg_replace('/\s+/','_',hc_filter($userInfo['nick']));
            $has_user = model('User')->getUserInfoByUsers($insertData['users']);
            if($has_user) {  // 判断用户名是否被注册
                $insertData['users'] = $insertData['users'].rand(100,999);
            }
